...
import { FEATURES, SECTION_NAMES } from "@/lib/constants";
import { getTopicListForSection } from "@/lib/mock-data";
import { paginatedItems, paginatedTotal } from "@/lib/api/pagination";
import { useTopics } from "@/hooks/use-forum";

const PAGE_SIZE = 10;

export function SectionPageView({ slug }: { slug: string }) {
  ...
  const { data: apiData, isLoading } = useTopics(
    FEATURES.forum ? slug : null,
    FEATURES.forum ? { page, sort } : undefined
  );

  const mockTopics = getTopicListForSection(slug);
  const allTopics = FEATURES.forum ? paginatedItems(apiData) : mockTopics;
  ...
  const totalPages = FEATURES.forum
    ? Math.max(1, Math.ceil(paginatedTotal(apiData, sorted.length) / PAGE_SIZE))
    : Math.max(1, Math.ceil(sorted.length / PAGE_SIZE));
  const topics = FEATURES.forum ? sorted : sorted.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  ...
  return (
    <div className="container px-4 py-8 md:px-6">
      ...
      <div className="rounded-lg border border-border">
        ...
        {FEATURES.forum && isLoading ? (
          <p className="p-8 text-center text-muted-foreground">Загрузка тем...</p>
        ) : topics.length === 0 ? (
